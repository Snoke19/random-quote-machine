...
export default async function fetchRandomQuoteByCategories(categories) {
    try {
        ...
        const response = await AxiosInstance.get(`/random/quote`, { params: { 'categories': categoriesStr } });
...
function handleError(error) {
    if (error.response) {
        ...
        throw new Error(`Failed to fetch quote: ${errorResponse.message || "Unknown error"}`, { cause: errorResponseInfo });
        ...
        if (error.message.includes('timeout')) {
            ...
            throw new Error("Something went wrong while fetching categories. Please try again.");
        }
    } else {
